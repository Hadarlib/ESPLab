...
void printSymbol()
{
    ...
    for (int i = 0; i < sectionsNum; i++)
    {
        ...
        {
            Elf32_Sym *symtab = (Elf32_Sym *)(map_start + shdr->sh_offset);
            int symbol_num = shdr->sh_size / shdr->sh_entsize;
            if(debugFlag){
                printf("Table Size: %d\n", symtab->st_size);
            ...
            if(debugFlag)
                printf("                       Name offset");
            printf("\n");
            Elf32_Shdr* symbol_names = (map_start + ehdr->e_shoff + sizeof(Elf32_Shdr) * shdr->sh_link);
            int name_offset = symbol_names->sh_offset;
            for (int j = 0; j < symbol_num; j++)
            {
                printf("[%-2d]     %08x   %5d                %-20s %-20s",
                       ...
                       map_start + name_offset + symtab[j].st_name);
                if(debugFlag)
                    printf("              %06d",symtab[j].st_name);
            printf("\n");
            }
        }
        shdr++;
